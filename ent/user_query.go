...
func (uq *UserQuery) QueryTicketEvents() *TicketEventQuery {
	query := &TicketEventQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withTicketEvents: uq.withTicketEvents.Clone(),
...
func (uq *UserQuery) WithTicketEvents(opts ...func(*TicketEventQuery)) *UserQuery {
	query := &TicketEventQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withTicketEvents = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withTicketEvents; query != nil {
		if err := uq.loadTicketEvents(ctx, query, nodes,
...
func (uq *UserQuery) loadTicketEvents(ctx context.Context, query *TicketEventQuery, nodes []*User, init func(*User), assign func(*User, *TicketEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.TicketEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(user.TicketEventsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
